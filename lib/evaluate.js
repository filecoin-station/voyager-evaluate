...
/**
 * @param {object} args
 * @param {import('./round').RoundData} args.round
...
export const evaluate = async ({
  ...
  const voyagerRoundDetails = await fetchRoundDetails(await ieContractWithSigner.getAddress(), roundIndex, recordTelemetry)
  // Omit the roundDetails object from the format string to get nicer formatting
  debug('ROUND DETAILS for round=%s', roundIndex, voyagerRoundDetails)

  const started = new Date()

  const fraudDetectionStats = await runFraudDetection(roundIndex, measurements, voyagerRoundDetails)
 ...
 * @param {import('./typings').RoundDetails} voyagerRoundDetails
 * @returns {Promise<import('./typings').FraudDetectionStats>}
 */
export const runFraudDetection = async (roundIndex, measurements, voyagerRoundDetails) => {
  //
  ...
  for (const m of measurements) {
    ...
    const isValidTask = voyagerRoundDetails.retrievalTasks.some(t => t.cid === m.cid)
    if (!isValidTask) {
  ...
  for (const m of measurements) {
    ...
    if (tasksPerNode.get(node) > voyagerRoundDetails.maxTasksPerNode) {
