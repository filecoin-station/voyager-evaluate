import { VOYAGER_API } from './config.js'
...
export const fetchRoundDetails = async (
  ...
  try {
    console.log('fetch', `${VOYAGER_API}/rounds/meridian/${contractAddress}/${roundIndex}`)
    const res = await fetch(`${VOYAGER_API}/rounds/meridian/${contractAddress}/${roundIndex}`)
    status = res.status
    if (!res.ok) {
      const msg = `Cannot fetch tasks for round ${contractAddress}/${roundIndex}: ${status}\n${await res.text()}`
      throw new Error(msg)
    }
    /** @type {import('./typings.js').RoundDetails} */
